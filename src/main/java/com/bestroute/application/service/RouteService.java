...
public class RouteService {
  ...
  private RouteResponse saveRoute(RouteRequest request, String jsonResponse) {
    try {
      ...
      throw new RouteGenerationException("AI servers are currently experiencing high demand. Please try again in a few moments.");

    } catch (Exception e) {
      log.error("Erro during IA response parse:", e);

      throw new RouteGenerationException("Failed to parse AI response into JSON structural map: " + e.getMessage());
    }
  }
